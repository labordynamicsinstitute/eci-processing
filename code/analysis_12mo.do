...
preserve
	...
	sleep 5000
	graph save   "${output}/eci_total_comp_allciv_wnotes.gph", replace
	...
	sleep 5000
	graph save   "${output}/eci_total_comp_allciv_nonotes.gph", replace	
	...
	export excel id series_title time value recession using "${output}/eci_total_comp_allciv.xls",  firstrow(varlabels) replace
	restore
...
preserve
	...
	export excel value recession using "${output}/eci_breakdown_all_civ.xls", sheet("breakdown") sheetmodify cell(D1) firstrow(varlabels)
	restore
//preserve
...
preserve
	...
	*sleep 5000
	graph save   "${output}/eci_total_comp_allprivate.gph", replace
...
preserve
	...
	*sleep 5000
	graph save   "${output}/eci_ws_allprivate.gph", replace
...
*	sleep 5000
	graph save   "${output}/eci_tc_allworkers.gph", replace
...
*	sleep 5000
	graph save   "${output}/eci_tc_allworkers_recent.gph", replace
...
*	sleep 5000
	graph save   "${output}/eci_tb_allworkers.gph", replace
...
*	sleep 5000
	graph save   "${output}/eci_tb_allworkers_recent.gph", replace
...
*	sleep 5000
	graph save   "${output}/eci_tc_regions.gph", replace
